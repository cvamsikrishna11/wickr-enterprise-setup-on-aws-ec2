...
REPLICATED_VERSION="2.56.0"
PINNED_DOCKER_VERSION="24.0.2"
...

Y29sb3I6ICM4ODc4ODA7Cn0K
CHANNEL_CSS_EOM
set -e
...
pullReplicatedImages() {
    if [ -n "$REGISTRY_ADDRESS_OVERRIDE" ]; then
        docker pull "${REGISTRY_ADDRESS_OVERRIDE}/${REGISTRY_PATH_PREFIX}replicated/replicated:stable-2.56.0"
        docker pull "${REGISTRY_ADDRESS_OVERRIDE}/${REGISTRY_PATH_PREFIX}replicated/replicated-ui:stable-2.56.0"
        (set -x; docker tag "${REGISTRY_ADDRESS_OVERRIDE}/${REGISTRY_PATH_PREFIX}replicated/replicated:stable-2.56.0" "${REPLICATED_REGISTRY_PREFIX}/replicated:stable-2.56.0")
        (set -x; docker tag "${REGISTRY_ADDRESS_OVERRIDE}/${REGISTRY_PATH_PREFIX}replicated/replicated-ui:stable-2.56.0" "${REPLICATED_REGISTRY_PREFIX}/replicated-ui:stable-2.56.0")
    else
        docker pull "${REPLICATED_REGISTRY_PREFIX}/replicated:stable-2.56.0"
        docker pull "${REPLICATED_REGISTRY_PREFIX}/replicated-ui:stable-2.56.0"
    fi
}
...
pullOperatorImage() {
    if [ -n "$REGISTRY_ADDRESS_OVERRIDE" ]; then
        docker pull "${REGISTRY_ADDRESS_OVERRIDE}/${REGISTRY_PATH_PREFIX}replicated/replicated-operator:2.56.0"
        (set -x; docker tag "${REGISTRY_ADDRESS_OVERRIDE}/${REGISTRY_PATH_PREFIX}replicated/replicated-operator:2.56.0" "${REPLICATED_REGISTRY_PREFIX}/replicated-operator:2.56.0")
    else
        docker pull "${REPLICATED_REGISTRY_PREFIX}/replicated-operator:2.56.0"
    fi
}
...
tagAndPushOperatorImage()  {
    docker tag \
        "${REPLICATED_REGISTRY_PREFIX}/replicated-operator:2.56.0" \
        "${1}/replicated/replicated-operator:2.56.0"
    docker push "${1}/replicated/replicated-operator:2.56.0"
}
...
tag_docker_images() {
    ...
    docker tag "$REPLICATED_REGISTRY_PREFIX/replicated:stable-2.56.0" "$REPLICATED_REGISTRY_PREFIX/replicated:current" \
        || docker tag -f "$REPLICATED_REGISTRY_PREFIX/replicated:stable-2.56.0" "$REPLICATED_REGISTRY_PREFIX/replicated:current"
    docker tag "$REPLICATED_REGISTRY_PREFIX/replicated-ui:stable-2.56.0" "$REPLICATED_REGISTRY_PREFIX/replicated-ui:current" \
        || docker tag -f "$REPLICATED_REGISTRY_PREFIX/replicated-ui:stable-2.56.0" "$REPLICATED_REGISTRY_PREFIX/replicated-ui:current"
}
...
ExecStartPre=/bin/mkdir -p /var/run/replicated /var/lib/replicated /var/lib/replicated/statsd /var/lib/replicated/retraced 
ExecStartPre=/bin/chown -R ${REPLICATED_USER_ID}:${DOCKER_GROUP_ID} /var/run/replicated /var/lib/replicated/branding
ExecStartPre=/bin/chown ${REPLICATED_USER_ID}:${DOCKER_GROUP_ID} /var/lib/replicated /var/lib/replicated/statsd /var/lib/replicated/retraced 
...
normal exit 0
pre-start script
    /bin/mkdir -p /var/run/replicated /var/lib/replicated /var/lib/replicated/statsd /var/lib/replicated/retraced 
    /bin/chown -R ${REPLICATED_USER_ID}:${DOCKER_GROUP_ID} /var/run/replicated /var/lib/replicated/branding
    /bin/chown ${REPLICATED_USER_ID}:${DOCKER_GROUP_ID} /var/lib/replicated /var/lib/replicated/statsd /var/lib/replicated/retraced 
...
install_operator() {
    ...
    if [ "$AIRGAP" != "1" ]; then
        ...
        echo -e "${URLGET_CMD} https://get.replicated.com${prefix}/operator?replicated_operator_tag=2.56.0${NC}"
        ${URLGET_CMD} "https://get.replicated.com${prefix}/operator?replicated_operator_tag=2.56.0" > "$REPLICATED_TEMP_DIR/operator_install.sh"
    fi
